"""Adversarial corpus loader and benchmark harness.

Each corpus case describes:
    - The CCPT-tagged inputs that would arrive at the proxy
    - The (adversarial) model response — what the model would do under attack
    - The expected AEGIS decision (ALLOW / BLOCK)

The benchmark replays each case through the orchestrator and reports per-category
...
def run_benchmark(cases: list[Case], mode: str = "balanced") -> dict[str, Any]:
    ...
    for case in cases:
        ...
        # Pre-flight (creates session, anchor, canaries; injects canary block).
        augmented, ctx = orch.pre_flight(req)

        # Optionally mint a capability for benign cases that explicitly request one.
        ...
        # Attach minted capability to any tool call that requests it.
        ...
        # Treat WARN as BLOCK for TP/FP accounting in strict-leaning analysis.
