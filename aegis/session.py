...
DEFAULT_KEY_FILENAME = ".aegis-master-key"
...
def _derive_master_key() -> bytes:
    """Source the master key from env, the working directory, or generate one.

    Resolution order:
        1. AEGIS_MASTER_KEY (hex-encoded 32 bytes)
        2. AEGIS_MASTER_KEY_FILE (path to a file containing the key)
        3. ./.aegis-master-key in the current working directory
        4. Fresh ephemeral key (dev fallback; sessions won't survive restart)
    """
    ...
    if os.path.exists(DEFAULT_KEY_FILENAME):
        return _read_key_file(DEFAULT_KEY_FILENAME)
...
class SessionStore:
    ...
    def iter_active(self) -> list[Session]:
        """Snapshot of non-expired sessions. Returns a list (caller-safe copy)."""
        now = time.time()
        with self._lock:
            return [s for s in self._sessions.values() if not (s.expires_at and s.expires_at < now)]
